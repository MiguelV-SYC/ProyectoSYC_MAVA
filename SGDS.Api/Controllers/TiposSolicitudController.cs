...
{
    var query = _context.TiposSolicitudes.Where(t => t.Activo).AsQueryable();
    ...
    [HttpPut("{id}")]
public async Task<IActionResult> ActualizarTipoSolicitud(int id, ActualizarTipoSolicitudDto dto)
{
    var esAdminSyc = User.FindFirst("esAdminSyc")?.Value == "True";
    if (!esAdminSyc)
        return Forbid();
