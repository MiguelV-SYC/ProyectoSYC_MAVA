...
export default function ListadoSolicitudesPage() {
  ...
  // Sin proyecto en contexto, no hay nada que mostrar todavía
  useEffect(() => {
    if (!proyectoId) return;
    getProyectosActivos().then((lista) => {
      setProyecto(lista.find((p) => p.id === proyectoId) ?? null);
    });
  ...
  }, [proyectoId, busqueda, estadoFiltro, tipoFiltro, pagina]);
        ...
        <main className="flex-1 flex items-center justify-center">
          <div className="text-center max-w-sm">
            <p className="text-sm text-ink-600 mb-3">
              Selecciona un proyecto en el panel "Mis proyectos" del menú lateral para ver sus solicitudes.
            </p>
          </div>
        </main>
      </div>
    ...
    <div className="flex min-h-screen bg-paper">
      ...
      <main className="flex-1 px-[38px] py-7 overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h1 className="font-display text-[19px] font-semibold text-ink-900">
              Solicitudes — {proyecto?.nombre ?? '...'}
            </h1>
            <p className="text-ink-600 text-[12.5px] mt-[3px]">
              {totalTodas} solicitudes registradas en este proyecto
            </p>
          </div>
          <button
        ...
        <div className="flex items-center gap-2.5 bg-white border border-line rounded-xl px-3.5 py-3 mb-4 flex-wrap">
          ...
          {/* "Operador asignado" y "Fecha" — pendientes de endpoint, visuales por ahora */}
          ...
          {conteos.map((c) => (
            <button
              ...
              className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 rounded-full ${
                estadoFiltro === c.estado
                  ? 'bg-[#0f172a] text-white'
                  : `border border-line ${ESTADO_STYLE[c.estado] ?? 'bg-white text-ink-600'}`
              }`}
            >
                  ...
                  >
                    ...
                    <td className="px-5 py-[13px] text-[13px] border-b border-line">
                      ...
                      <div className="text-[11px] text-ink-400">
                        {s.ciudadanoDocumento ?? s.empresaNit ?? ''}
                      </div>
                    </td>
                    <td className="px-5 py-[13px] text-[13px] border-b border-line">
